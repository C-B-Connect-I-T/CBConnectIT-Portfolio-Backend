...
import com.cbconnectit.instrumentation.JobPositionInstrumentation.givenAJobPosition
...
import com.cbconnectit.instrumentation.JobPositionInstrumentation.givenJobPositionList
...
class JobPositionRoutingTest : BaseRoutingTest() {
    ...
    fun `when fetching all jobPositions, we return a list`() = withBaseTestApplication(
        ...
        coEvery { jobPositionController.getJobPositions() } returns givenJobPositionList().map { it.toDto() }

        val response = doCall(HttpMethod.Get, "/job_positions")

        assertThat(response.status).isEqualTo(HttpStatusCode.OK)
        assertThat(response.parseBody<List<JobPositionDto>>()).hasSize(4)
    }
    // </editor-fold>
    ...
    fun `when fetching a specific jobPosition that exists by id, we return that jobPosition`() = withBaseTestApplication(
        ...
        val jobPositionResponse = givenAJobPosition().toDto()
    ...
    fun `when updating jobPosition with successful insertion, we return response jobPosition body`() = withBaseTestApplication(
        ...
        val body = toJsonBody(givenAValidUpdateJobPosition())
